 ...
 * @return        < 0 - Error
 */
int ltn_nal_h265_findHeader(const uint8_t *buf, int lengthBytes, int *offset);

char *ltn_nal_hevc_findNalTypes(const uint8_t *buf, int lengthBytes);

char *ltn_nal_h265_findNalTypes(const uint8_t *buf, int lengthBytes);

const char *h265Nals_lookupName(int nalType);

/**
 * @brief         A machanism to find h265 slices in a bitstream, count the number of respective I/P/B frames.
 ...
 * @param[in]     void *ctx - Context returned from the prior h265_slice_counter_alloc() call.
...
/**
 * @brief         Print the current I/P/B slice counts to a file descriptor.
 * @param[in]     void *ctx - Context returned from the prior h265_slice_counter_alloc() call.
 ...
 */
void h265_slice_counter_write(void *ctx, const unsigned char *pkts, int packetCount);

struct h265_slice_counter_results_s
{
    uint64_t i;
    uint64_t b;
    uint64_t p;
...
#define H265_SLICE_COUNTER_HISTORY_LENGTH 20
