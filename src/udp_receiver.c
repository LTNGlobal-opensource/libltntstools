	...
	int stripRTPHeader)
{
	...
	struct ltntstools_udp_receiver_s *ctx = (struct ltntstools_udp_receiver_s *)calloc(1, sizeof(*ctx));
...
int ltntstools_udp_receiver_join_multicast(struct ltntstools_udp_receiver_s *ctx, char *ifname)
{
	assert(ctx && ifname);
	if (!IN_MULTICAST(ntohl(ctx->sin.sin_addr.s_addr)))
		return -1;
...
int ltntstools_udp_receiver_drop_multicast(struct ltntstools_udp_receiver_s *ctx, char *ifname)
{
	assert(ctx && ifname);
	if (!IN_MULTICAST(ntohl(ctx->sin.sin_addr.s_addr)))
		return -1;
