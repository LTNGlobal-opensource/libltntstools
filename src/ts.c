...
int ltntstools_verifyPacketWith64bCounter(unsigned char *pkt, int lengthBytes, uint16_t pid, uint64_t lastCounter, uint64_t *currentCounter)
{
	...
	/* Bytes 16..187 are stuffing (0xff) in every packet generated by
	 * ltntstools_generatePacketWith64bCounter(). Compare directly against
	 * that constant instead of a lazily-initialized static buffer, which
	 * was a data race when this function is called from multiple threads.
	 */
	for (int i = 16; i < 188; i++) {
		if (*(pkt + i) != 0xff)
			return -1;
	}
